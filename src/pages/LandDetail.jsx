...
export default function LandDetail() {
  ...
  useEffect(() => {
    landApi
      .getBySlug(slug)
      .then((data) => {
        if (data) setLand(data);
      })
      .catch(() => {
        // Fallback to static constant if backend doesn't have it
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const detail = slug && LAND_DETAILS[slug] ? LAND_DETAILS[slug] : (land ? {
    ...
    amenities: (Array.isArray(land.amenities) && land.amenities.length > 0
      ? land.amenities
      : ["Road Access", "Electricity Grid", "Good Drainage", "Flat Topography"]
    ).map((a) => {
