...
export default function AIAssistantPanel({ brief }) {
  ...
  useEffect(() => {
    ...
    if (key === lastBriefKey.current && matches.length > 0) return;
    lastBriefKey.current = key;

    recommendContractors(brief, { limit: 3 })
      .then((res) => {
  ...
  async function handleSend(e) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `ai-${Date.now()}`, sender: "ai", text: res?.reply || "Got it! Matching you with contractors." },
      ]);
    } catch (err) {
      console.warn("AI Assistant interaction error:", err);
      setMessages((prev) => [
        ...prev,
        { id: `ai-${Date.now()}`, sender: "ai", text: "I have recorded your requirements and updated your matching contractors." },
      ]);
    } finally {
      setIsThinking(false);
    }
      ...
      <div className="relative p-5 sm:p-6">
        ...
        <div className="mt-4 grid gap-2.5 sm:grid-cols-3">
          {(Array.isArray(matches) ? matches : [])
            .filter((m) => m && m.contractor)
            .map(({ contractor, reasons = [] }) => (
              <Link
                key={contractor.slug || contractor.id || contractor.name}
              ...
              >
                ...
                <div className="mt-1 flex items-center gap-1.5">
                  <StarRating value={contractor.rating || 4.8} />
                  <span className="text-xs text-white/80">{contractor.rating || 4.8}</span>
                </div>
                <p className="mt-1.5 line-clamp-2 text-[11px] text-forest-100/75">
                  {reasons?.[0] || "Verified Professional on TerraMatch"}
                </p>
              </Link>
            ))}
        </div>
